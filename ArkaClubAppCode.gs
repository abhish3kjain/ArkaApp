...
function getAppMasterData() {
  ...
  // 8. LOAD BOOK POST DB ---
  const bookPostSheet = ss.getSheetByName("BookPostDB");
  const bookPostDB = [];
  if (bookPostSheet) {
    const bpData = bookPostSheet.getDataRange().getValues();
    for (let i = 1; i < bpData.length; i++) {
      if (bpData[i][0]) {
        let rawDate = bpData[i][3];
        let safeDateString = rawDate instanceof Date ? Utilities.formatDate(rawDate, Session.getScriptTimeZone(), "dd-MM-yyyy HH:mm:ss Z") : String(rawDate);

        bookPostDB.push({
          postId: bpData[i][0].toString(),
          bookId: bpData[i][1].toString(),
          memberId: bpData[i][2].toString(),
          timestamp: safeDateString,
          postType: bpData[i][4].toString(),
          content: bpData[i][5].toString(),
          status: bpData[i][6].toString(),
          likeCount: Number(bpData[i][7]) || 0
        });
      }
    }
  }
...
/**
 * Creates a new community discussion post attached to a specific book.
 * Generates a unique BookPostID safely using LockService and awards 
 * club points to the member via the batch logger.
 * * @param {Object} postData - The payload from the frontend.
 * @param {string} postData.memberId - The MemberID of the author.
 * @param {string} postData.bookId - The ID of the book being discussed.
 * @param {string} postData.postType - The category ("General Note", "Quote I Loved", "Fan Cast").
 * @param {string} postData.content - The text content (preserves \n formatting).
 * @returns {Object} Response object containing status, message, and the new post data.
 */
function createBookPost(postData) {
  if (!postData.memberId || !postData.bookId || !postData.content) {
    return { status: "error", message: "Missing required post information." };
  }

  const lock = LockService.getScriptLock();
  if (!lock.tryLock(8000)) {
    return { status: "error", message: "System busy. Could not publish post. Please try again." };
  }

  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const postSheet = ss.getSheetByName("BookPostDB"); // Your newly created sheet
    
    // 1. Generate new ID
    const postDataRange = postSheet.getDataRange().getValues();
    let newPostNum = 1;
    if (postDataRange.length > 1) {
      let lastIdString = postDataRange[postDataRange.length - 1][0]; 
      let lastNum = parseInt(lastIdString.split('_')[2]);
      if (!isNaN(lastNum)) newPostNum = lastNum + 1;
    }
    let newBookPostId = "ARKA_BOOKPOST_" + newPostNum;

    // 2. Format Timestamp
    const timestamp = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "dd-MM-yyyy HH:mm:ss Z");

    // 3. Prepare the row based on the finalized schema
    const newRow = [
      newBookPostId,        // Col A: BookPostID
      postData.bookId,      // Col B: BookID
      postData.memberId,    // Col C: MemberID
      timestamp,            // Col D: Timestamp
      postData.postType,    // Col E: PostType
      postData.content,     // Col F: Content (Raw string, preserves \n)
      "Active",             // Col G: Status
      0                     // Col H: LikeCount (Starts at 0)
    ];

    // 4. Write to DB
    postSheet.appendRow(newRow);

    // 5. Award Points (Using the previously created batch logger)
    let newActivitiesQueue = [
      { typeId: "ARKA_ACTTYP_BOOKPOST", val: 1, desc: newBookPostId }
    ];
    let loggedResults = logActivityBatch(postData.memberId, newActivitiesQueue);

    return { 
      status: "success", 
      message: "Post published successfully!", 
      newActivities: loggedResults,
      postDetails: {
        postId: newBookPostId,
        timestamp: timestamp
      }
    };
    ...
    return { status: "error", message: "Failed to publish post due to a system error." };
  } finally {
    lock.releaseLock();
  }
}
...
/**
 * Retrieves all active discussion posts for a specific book to be displayed 
 * on the book's details page. Reverses the array to show the newest posts first.
 * * @param {string} targetBookId - The BookID to filter posts by.
 * @returns {Array<Object>} An array of formatted post objects ready for UI rendering.
 */
function getBookPosts(targetBookId) {
  if (!targetBookId) return [];

  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const postSheet = ss.getSheetByName("BookPostDB");
    const data = postSheet.getDataRange().getValues();
    
    // We will need Member details to display avatars and names alongside the posts
    const memSheet = ss.getSheetByName(MEMBERS_SHEET);
    const memData = memSheet.getDataRange().getValues();
    
    // Build a quick lookup map for Member names and avatars (Col 1 is ID, Col 2 is Name, Col 9 is Avatar URL)
    // Adjust column indices based on your exact MemberDB structure
    let memberMap = {};
    for (let i = 1; i < memData.length; i++) {
      if (memData[i][0]) {
        memberMap[memData[i][0]] = {
          name: memData[i][1] || "Unknown Member",
          avatar: memData[i][8] || "" 
        };
      }
    }
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      // Filter by the specific book and ensure the post wasn't deleted
      if (bookId === targetBookId && status === "Active") {
        let memberId = data[i][2];
        let authorInfo = memberMap[memberId] || { name: "Unknown", avatar: "" };
...
/**
 * Retrieves and groups the reading status of all members who have a specific book on their shelf.
 * Dynamically selects the most relevant date to display based on the member's reading status 
 * (e.g., Finished Date for completed books, Updated Date for currently reading).
 * * @param {string} targetBookId - The unique identifier of the book (from ArkaLibraryDB).
 * @returns {Object} An object grouping members by their reading status, including formatted dates.
 * Format: { 
 * "Reading": [{name, avatar, dateLabel, dateString}], 
 * "Finished": [...], 
 * "To Read": [...] 
 * }
 */
function getBookShelfStatusWithDates(targetBookId) {
  if (!targetBookId) return {};

  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    
    // 1. Fetch all Member data for avatars and names
    const memSheet = ss.getSheetByName(MEMBERS_SHEET);
    const memData = memSheet.getDataRange().getValues();
    ...
    for (let i = 1; i < memData.length; i++) {
      if (memData[i][0]) {
        memberMap[memData[i][0]] = {
          name: memData[i][1] || "Unknown",
          // Assuming Avatar URL is in Col I (Index 8). Update index if different!
          avatar: memData[i][8] || "" 
        };
      }
    }

    // 2. Fetch Shelf data to find who has this book and their dates
    const shelfSheet = ss.getSheetByName(SHELF_SHEET);
    const shelfData = shelfSheet.getDataRange().getValues();
    
    // We use a map to store the LATEST record per member so historical edits don't cause duplicates
    let latestMemberShelfRecord = {};

    for (let i = 1; i < shelfData.length; i++) {
      let memberId = shelfData[i][1]; // Col B
      let bookId = shelfData[i][2];   // Col C

      if (bookId === targetBookId && memberId) {
        // Overwrites older rows, ensuring we only keep their most recent status update
        latestMemberShelfRecord[memberId] = {
          status: shelfData[i][3],       // Col D
          dateAdded: shelfData[i][6],    // Col G
          dateUpdated: shelfData[i][7],  // Col H
          dateFinished: shelfData[i][8]  // Col I
        };
      }
    }

    // 3. Group the members and apply the specific date rules
    let groupedStatus = {
      "Reading": [],
      "Finished": [],
      "To Read": [],
    ...
    /**
     * Helper to safely format Google Sheet dates into short, clean strings for the UI
     * @param {Date|string} rawDate - The date from the sheet
     * @returns {string} Formatted date (e.g., "12-Oct" or original string if not a date)
     */
    function formatShortDate(rawDate) {
      if (!rawDate) return "";
      if (rawDate instanceof Date) {
        // e.g., "14-Oct-2026" -> split to keep it short if desired, or just use it.
        return Utilities.formatDate(rawDate, Session.getScriptTimeZone(), "dd-MMM");
      }
      return String(rawDate);
    }

    // 4. Process each member's latest record and figure out which date to show
    for (let memberId in latestMemberShelfRecord) {
      let record = latestMemberShelfRecord[memberId];
      let memberInfo = memberMap[memberId] || { name: "Unknown", avatar: "" };
      
      let displayDateLabel = "";
      let displayDateValue = "";

      // Apply your specific rules for which date matters most
      if (record.status === "Finished") {
        displayDateLabel = "Fin:";
        displayDateValue = formatShortDate(record.dateFinished || record.dateUpdated);
      } 
      else if (record.status === "Reading") {
        displayDateLabel = "Upd:";
        displayDateValue = formatShortDate(record.dateUpdated);
      } 
      else if (record.status === "To Read" || record.status === "Did Not Finish") {
        displayDateLabel = "Add:";
        // Prefer update date if they bumped it, otherwise fall back to added date
        displayDateValue = formatShortDate(record.dateUpdated || record.dateAdded);
      }

      // Push the compiled data into our final group
      if (groupedStatus[record.status]) {
        groupedStatus[record.status].push({
          name: memberInfo.name,
          avatar: memberInfo.avatar,
          dateLabel: displayDateLabel,
          dateString: displayDateValue
        });
      }
    }
    ...
    console.error("Error fetching club shelf status with dates: ", error);
    // Return empty arrays so the frontend doesn't crash on a failure
    return { "Reading": [], "Finished": [], "To Read": [], "Did Not Finish": [] };
  }
}
